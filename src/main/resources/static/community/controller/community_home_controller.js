$(document).ready(function () {
    ...
    $.ajax({
        ...
        data: {userID: window.localStorage.getItem("user_id")},
        async: false,
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                var new_history = "<li id='reco" + i + "'>\n" +
                    "                            <a rel=\"nofollow\">\n" +
                    ...
                    "                            </a>\n" +
                    "                            <a rel=\"nofollow\" class=\"title\">" + data[i].postname + "</a>\n" +
                    "                        </li>";
                $(".module-list").append(new_history);
                $("#reco" + i).click(function () {
                    var id = $(this).attr("id").substr(4);
                    console.log(data[id]);
                    localStorage.setItem("post_detail_id", data[parseInt(id)].postid);
                    window.location.href = "community_detail";
                });
            }
        },
